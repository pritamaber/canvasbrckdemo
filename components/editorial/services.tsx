            ...
            <Reveal key={s.name} delay={0.05 * i}>
              <a href="#contact" className="group block">
                <div className="relative aspect-[4/3] overflow-hidden rounded-sm">
                  ...
                  />
                  {s.comingSoon && (
                    <span className="absolute left-4 top-4 rounded-full bg-forest px-3.5 py-1.5 text-[9px] font-bold uppercase tracking-[0.24em] text-ivory">
                      Coming soon
                    </span>
                  )}
                </div>
                <div className="mt-5 flex items-center justify-between gap-4">
                  <h3 className="text-lg font-bold text-forest transition-colors duration-300 group-hover:text-sky">
                    {s.name}
                  </h3>
                  <ArrowUpRight
                    size={18}
                    strokeWidth={1.5}
                    className="shrink-0 text-fern/45 transition-all duration-500 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-sky"
                  />
                </div>
